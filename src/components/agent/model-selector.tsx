...
export function ModelSelector() {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="truncate max-w-[120px]">{MODEL_CONFIG[model].name}</span>
        <ChevronDown className={`h-3 w-3 transition-transform text-white/40 ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <>
          <div className="fixed inset-0 z-50" onClick={() => setOpen(false)} />
          <div className="absolute bottom-full left-0 z-50 mb-2 min-w-[260px] rounded-xl border border-white/10 p-1.5 shadow-2xl" style={{ backgroundColor: 'hsl(30, 3.3%, 11.8%)' }}>
            {models.map((m) => (
              <button
                key={m}
                onClick={() => {
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[11px] text-white/40">{MODEL_CONFIG[m].description}</div>
                </div>
                <span className="text-[10px] text-white/20">{MODEL_CONFIG[m].provider}</span>
              </button>
            ))}
          </div>
        </>
